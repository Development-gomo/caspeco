// src/components/major/HeaderBackup.jsx
...
export default function HeaderBackup({
  ...
  const [altLangUrl, setAltLangUrl] = useState(
    langHome(SUPPORTED_LANGS.filter((l) => l !== lang)[0]),
  );
  ...
  const headerClasses = scrolled
    ? "fixed top-0 w-full z-50 text-black transition-all duration-300 py-4 bg-white shadow-sm"
  ...
  useEffect(() => {
    async function fetchAltLangUrl() {
      try {
        ...
        if (entryId) {
          const translations = await getEntryTranslations(
            entryId,
            entryType,
            lang,
          );
    ...
    <header className={headerClasses}>
      <div className="web-width mx-auto px-6 flex items-center justify-between relative">
        ...
        <nav className="hidden lg:flex items-center gap-4 ">
          ...
          <div
            className="px-8 py-4 flex items-center gap-8 lg:absolute lg:left-[338px]
            "
          >
                ...
                menu.main.map((item) => (
                  <li key={item.id} className="relative group">
                    <Link
                      ...
                      prefetch={true}
                      className={`
                            ${scrolled ? "text-black/90 hover:text-black" : "text-white/90 hover:text-white"} relative z-9 text-[15px] transition leading-[18px] flex items-center gap-2`}
                    >
                      {item.title}
        ...
        {mobileOpen && (
          <div
            className="fixed h-[100vh] inset-0 z-[60] bg-black/60 backdrop-blur-sm lg:hidden"
            onClick={() => {
              setMobileOpen(false);
              setOpenSubmenu(null);
            }}
          >
            <div
              className="absolute right-0 top-0 h-full w-72 bg-(--color-brand) shadow-xl flex flex-col"
